...
usage() {
  cat <<'EOF'
Usage:
  fetch_mpv_headers.sh --output-dir <dir> [--work-dir <dir>] [--mpv-version <version>]
EOF
}
...
stage_dir="$(mktemp -d "${WORK_DIR%/}/mpv-headers.XXXXXX")"
...
cp "${header_source_dir}"/*.h "${OUTPUT_DIR}/mpv/"

echo "Fetched mpv headers to ${OUTPUT_DIR}/mpv"
